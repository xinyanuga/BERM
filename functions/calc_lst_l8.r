...
##Note might also use veg.em=0.98, and maybe veg.cut of 0.45
calc_lst<- function(lsat, water.cut=0.05, soil.cut=0.15,  veg.cut=0.4, water.em= 0.991, soil.em=0.969, veg.em=0.976){
  ...
  out<-data.frame(pv=pv, ndvi=lsat$ndvi)
  ...
  out$emiss[out$ndvi>=veg.cut]<- veg.em+0.005
  
  ## emiss of marsh soil in aster was 0.969
  out$emiss[out$ndvi<=soil.cut&out$ndvi>= water.cut]<-soil.em
  ##for mixed soil and vegetation pixels, weight the emissivity by the proportion of each (pv is proportion vegetation)
  out$emiss[out$ndvi<veg.cut&out$ndvi> soil.cut]<- soil.em*(1-out$pv[out$ndvi<veg.cut&out$ndvi> soil.cut])+
    veg.em*out$pv[out$ndvi<veg.cut&out$ndvi> soil.cut]+0.005
